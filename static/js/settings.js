...
window.SettingsManager = {
    ...
    async testRconConnection() {
        ...
        try {
            const data = await apiCall('/api/test/rcon', {
                method: 'POST',
                body: JSON.stringify({ host, port })
            });

            if (data.success) {
                showToast('RCON connection test successful', 'success');
            } else {
    ...
    async testFtpConnection() {
        ...
        if (!host) {
            showToast('Please enter FTP host first', 'error');
            return;
        }
        ...
        try {
            const data = await apiCall('/api/test/ftp', {
                method: 'POST',
                body: JSON.stringify({ host, port })
            });
