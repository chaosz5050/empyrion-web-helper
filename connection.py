...
class EmpyrionConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # Send authentication
            logger.info(f"Sending authentication...")
            ...
            try:
                ...
                if auth_response:
                    ...
                    if "Logged in successfully" in auth_response:
                        # Mark as connected BEFORE testing with help command
                        self.is_connected = True
                        logger.info("Authentication successful, connection established")
                        
                        # Optional: Test with help command
                        ...
                        if test_result:
                            ...
                            logger.warning("Help command didn't return data, but auth was successful")
                        
                        return True
                    else:
                        logger.error("Authentication failed - no success message")
                        self.disconnect()
                        return False
            except Exception as e:
